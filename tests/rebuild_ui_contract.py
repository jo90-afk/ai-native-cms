...
def main()->None:
    rebuild=text('api/content-rebuild.php');reconcile=text('database/reconcile.php');auth=text('api/cms-auth.php');login=text('cms/index.php');pages=text('cms/pages.php');js=text('cms/cms.js');css=text('cms/cms.css');config=text('config/site.example.php')
    ...
    require(pages,['requireCmsAuth(false)','meta name="cms-csrf"','/cms/cms.js','/cms/cms.css'],'page editor shell');require(login,['cmsCurrentUser','/cms/cms.js','/cms/cms.css'],'login shell')
    ...
    require(js,['X-CMS-CSRF','/api/cms-auth.php','/api/cms-pages.php','dataset.hash','dataset.original','response.status === 401'],'CMS browser client')
    if '.innerHTML' in js or 'insertAdjacentHTML' in js or 'document.write' in js: fail('CMS browser client introduced an HTML injection rendering path')
    if 'http://' in js or 'https://' in js: fail('CMS browser client introduced a third-party/network origin')
