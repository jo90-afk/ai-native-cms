...
function readinessFilesystemChecks(string $root): array {
    ...
    $checks[]=readinessCheck('filesystem.repository-pages','Repository page projection',$pageFailures?'fail':'pass',$pageFailures?count($pageFailures).' configured repository page(s) are missing, outside the public root, or not writable.':'All configured repository pages are writable inside the public root.');
    $rootWritable=is_dir($root)&&is_writable($root);$checks[]=readinessCheck('filesystem.page-create','CMS-created page routes',$rootWritable?'pass':'fail',$rootWritable?'The public root is writable for bounded CMS-created root-level pages.':'The public root is not writable, so Composer cannot create new managed page routes.');
    ...
    $writingTargets=[];$route=trim(str_replace('\\','/',(string)siteConfigValue('writing','route_root','writing')),'/');if($route!=='')$writingTargets[]=$route.'/readiness-probe/index.html';$index=trim((string)siteConfigValue('writing','index_path','content/posts/index.json'));if($index!=='')$writingTargets[]=$index;$writingFailures=[];foreach($writingTargets as $path)if(!readinessWritableTarget($root,$path,false))$writingFailures[]=$path;
    $checks[]=readinessCheck('filesystem.writing','Writing projection targets',$writingFailures?'fail':'pass',$writingFailures?count($writingFailures).' writing projection target(s) cannot be written safely.':'Writing article and index targets have writable in-root paths.');
...
function readinessDatabaseChecks(string $root): array {
    ...
    try{$state=bootstrapState($pdo,$root);$status=(string)$state['status'];
        ...
        elseif(bootstrapRepairable($state))$checks[]=readinessCheck('database.bootstrap','Schema and owner bootstrap','fail','The database is an incomplete current-schema installation. Inspect it, then use database/bootstrap.php --repair only if completion is intended.');
        else $checks[]=readinessCheck('database.bootstrap','Schema and owner bootstrap','fail','The database schema is partial or older than this repository. Bootstrap repair will not upgrade it; use an explicit migration path.');
