...
try{
    ...
    if($method!=='POST')runtimeJson(['ok'=>false,'error'=>'Method not allowed.'],405);requireSameOrigin(true);requireCmsCsrf();enforceRateLimit('cms-branding',60,3600,$root);$payload=readJsonBody(131072);$settings=$payload['settings']??null;$expected=trim((string)($payload['expectedHash']??''));if(!is_array($settings))runtimeJson(['ok'=>false,'error'=>'Branding settings are required.'],422);runtimeJson(['ok'=>true,'branding'=>brandingSave($root,$settings,$expected)]);
