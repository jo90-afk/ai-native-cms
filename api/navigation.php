...
function navigationDefaultItems(string $root): array {
    ...
    $items=[];foreach(cmsConfiguredPages($root) as $path=>$label){$items[]=['id'=>'page-'.substr(hash('sha256',$path),0,10),'label'=>$label,'href'=>$path==='index.html'?'/':'/'.$path,'enabled'=>true];if(count($items)>=12)break;}
...
function navigationAddPage(string $root,string $path,string $label): array {
    $state=navigationState($root);$href='/'.ltrim($path,'/');foreach($state['items'] as $item)if((string)$item['href']===$href)return $state;$items=$state['items'];$items[]=['id'=>'page-'.substr(hash('sha256',$path),0,10),'label'=>substr(trim($label),0,80),'href'=>$href,'enabled'=>true];return navigationSave($root,$items,$state['hash']);
}
