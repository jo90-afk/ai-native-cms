...
function onboardingDatabaseSummary(string $root,array $readiness): array {
    $out=['available'=>false,'schemaCurrent'=>false,'brandingConfigured'=>false,'navigationConfigured'=>false,'publishedPosts'=>0,'managedPages'=>count(cmsConfiguredPages($root))];
    try{
        if(!dbConfigured()||!extension_loaded('pdo_mysql'))return $out;$health=dbHealth();$out['available']=true;$out['schemaCurrent']=(int)($health['schemaVersion']??0)>=8;if(!$out['schemaCurrent'])return $out;
...
function onboardingState(string $root): array {
    ...
    $bootstrap=onboardingCheckPassed($checks,'database.bootstrap');$authority=onboardingCheckPassed($checks,'content.authority');
    $steps=[];
    ...
    $steps[]=onboardingStep('bootstrap','Initialize secure CMS state',$bootstrap?'complete':'blocked',$bootstrap?'Schema v8 and a persisted CMS owner are ready.':'Finish the CLI database/owner bootstrap before browser authoring.','/cms/readiness.php','See readiness');
    $steps[]=onboardingStep('content','Initialize editable page content',$authority?'complete':($bootstrap?'ready':'blocked'),$authority?'Configured repository pages are initialized in canonical SQL.':($bootstrap?'Run the explicit initial repository reconciliation, then edit page copy.':'Canonical content initialization waits for database bootstrap.'),'/cms/pages.php','Open Pages');
    $steps[]=onboardingStep('pages','Shape the starter pages',$authority?'ready':'blocked',$authority?'Edit starter copy or compose additional pages from trusted repository templates.':'Initialize canonical page content first.','/cms/pages.php','Edit pages');
    $steps[]=onboardingStep('branding','Make the visual identity yours',$db['brandingConfigured']?'complete':($bootstrap?'ready':'blocked'),$db['brandingConfigured']?'Canonical branding has been customized.':'Set the site name/mark and bounded starter design tokens.','/cms/branding.php','Open Branding');
    $steps[]=onboardingStep('navigation','Confirm how people move through the site',$db['navigationConfigured']?'complete':($bootstrap?'ready':'blocked'),$db['navigationConfigured']?'Primary navigation is canonical and configured.':'The starter works with derived navigation; save the order/labels when you are ready.','/cms/navigation.php','Open Navigation');
    ...
    return [
        'ready'=>(bool)$readiness['ready']&&$siteConfig&&$starterReady&&$bootstrap&&$authority,
        ...
        'principles'=>[
            ...
            'Accepted authored content/state stays canonical in MySQL.',
