...
try{
    ...
    if($method!=='POST')runtimeJson(['ok'=>false,'error'=>'Method not allowed.'],405);requireSameOrigin(true);requireCmsCsrf();enforceRateLimit('cms-navigation',60,3600,$root);$payload=readJsonBody(131072);$items=$payload['items']??null;$expected=trim((string)($payload['expectedHash']??''));if(!is_array($items))runtimeJson(['ok'=>false,'error'=>'Navigation items are required.'],422);runtimeJson(['ok'=>true,'navigation'=>navigationSave($root,$items,$expected)]);
