...
try{
    ...
    if($method==='GET'){
        ...
        if($path===''){
            $items=[];foreach($managed as $p=>$label){$full=cmsSafePublicFile($root,$p);if($full!==null)$items[]=cmsPublicPageInfo($root,$p,$label);}
            runtimeJson(['ok'=>true,'pages'=>$items,'contentAuthority'=>$authority]);
        }
        if(!isset($managed[$path]))runtimeJson(['ok'=>false,'error'=>'Page not found.'],404);
        $full=cmsSafePublicFile($root,$path);if($full===null)runtimeJson(['ok'=>false,'error'=>'Page not found.'],404);
    ...
    $payload=readJsonBody(1500000);$path=trim((string)($payload['path']??''));if(!isset($managed[$path]))runtimeJson(['ok'=>false,'error'=>'Page not found.'],404);
    $changes=$payload['changes']??[];if(!is_array($changes)||count($changes)>300)runtimeJson(['ok'=>false,'error'=>'Invalid page changes.'],422);$validated=[];
    foreach($changes as $change){
        if(!is_array($change))continue;$id=trim((string)($change['id']??''));$replacement=(string)($change['html']??'');$hash=(string)($change['hash']??'');
        if($id===''||strlen($id)>191||!preg_match('/^[A-Za-z0-9._:-]+$/',$id))runtimeJson(['ok'=>false,'error'=>'Invalid editable block identifier.'],422);
        if(strlen($replacement)>100000)runtimeJson(['ok'=>false,'error'=>'One edited text block is too large.'],413);$validated[]=['id'=>$id,'html'=>$replacement,'hash'=>$hash];
    }
    if(!$validated)runtimeJson(['ok'=>true,'saved'=>0,'page'=>cmsPublicPageInfo($root,$path,$managed[$path]),'contentAuthority'=>$authority]);
    if(!(bool)($authority['ready']??false)){contentAuthorityImport($root,false,'cms-bootstrap');$authority=contentAuthorityStatus($root);}
    ...
    runtimeError($e,'The page edit could not be saved.',500);
}
