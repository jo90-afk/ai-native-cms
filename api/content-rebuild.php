...
require_once __DIR__.'/llms-projection.php';
...
/** Site-wide finalization remains schema-v8 compatible across the explicit upgrade seam. */
function contentFinalizePublicProjections(string $root,?array $hooks=null,?array $seedContext=null): array {dbRequireSchemaVersion(8);$hooks=$hooks??contentRebuildHooks($root);$context=$seedContext??['startedAt'=>gmdate('c'),'hooks'=>[],'core'=>[]];$runs=[];foreach(contentRebuildRunHooks($hooks,'after_pages',$root,$context) as $run)$runs[]=$run;$context['core']['seo']=seoProjectAllPublicPages($root);foreach(contentRebuildRunHooks($hooks,'after_seo',$root,$context) as $run)$runs[]=$run;$context['core']['navigation']=navigationProject($root);$context['core']['branding']=brandingProject($root);$context['core']['redirects']=redirectProject($root);foreach(contentRebuildRunHooks($hooks,'finalize',$root,$context) as $run)$runs[]=$run;$context['core']['routes']=pageRoutesEnabled()?pageProjectionProjectManagedCleanRoutes($root,array_keys(contentAuthorityManagedPages($root))):['enabled'=>false,'managed'=>0,'projected'=>0,'rewritten'=>0,'references'=>0,'metadata'=>0];$context['core']['discovery']=discoveryProject($root);$context['core']['llms']=llmsProject($root);$context['finishedAt']=gmdate('c');return ['core'=>$context['core'],'hooks'=>$runs,'finishedAt'=>$context['finishedAt'],'context'=>$context];}
/**
 * Full rebuild is dual-readable across the upgrade seam: schema v8 compositions
 * project through their trusted page_block_templates, while schema v9+ projects
 * through canonical block_presets. New composition mutation remains v9+-only.
 */
function contentRebuild(string $root): array {dbRequireSchemaVersion(8);$hooks=contentRebuildHooks($root);$context=['startedAt'=>gmdate('c'),'hooks'=>[],'core'=>[]];$runs=[];foreach(contentRebuildRunHooks($hooks,'before_documents',$root,$context) as $run)$runs[]=$run;$context['core']['documents']=contentAuthorityProjectConfiguredDocuments($root);foreach(contentRebuildRunHooks($hooks,'after_documents',$root,$context) as $run)$runs[]=$run;foreach(contentRebuildRunHooks($hooks,'before_pages',$root,$context) as $run)$runs[]=$run;$context['core']['pages']=contentAuthorityProjectPages($root);$context['core']['compositions']=compositionProjectAll($root);$context['core']['publishing']=projectPublishedPosts($root);$final=contentFinalizePublicProjections($root,$hooks,$context);foreach($final['hooks'] as $run)$runs[]=$run;$context=$final['context'];return ['ok'=>true,'core'=>$context['core'],'hooks'=>$runs,'finishedAt'=>$context['finishedAt']];}
