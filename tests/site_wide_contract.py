...
def text(path: str) -> str:
    file = ROOT / path
    if not file.is_file():
        fail(f"missing M-006 file: {path}")
...
def main() -> None:
    ...
    navigation = text("api/navigation.php")
    navigation_api = text("api/cms-navigation.php")
    branding = text("api/branding.php")
    branding_api = text("api/cms-branding.php")
    ...
    if "foreach(contentAuthorityManagedPages($root) as $path=>$label)" in authority.split("function contentAuthorityDocumentSpecs",1)[1].split("function contentAuthorityLogChange",1)[0]:
        fail("dynamic managed pages re-entered repository page-source lineage")
    require(composer, ["foreach(cmsConfiguredPages($root) as $path=>$label)"], "repository-owned template harvesting")
    for stale in ["function composerSaveComposition", "function composerComposition(", "function composerProjectPage"]:
        if stale in composer:
            fail("duplicate composition persistence remains in template module")

    require(store, [
        "compositionValidateParent",
        "Page hierarchy cannot contain a parent cycle",
        "compositionSafeNewPagePath",
        "compositionCreatePage",
        "compositionRetargetShell",
        "parent_path",
        "shell_path",
        "cmsSafePublicTarget",
    ], "canonical new-page hierarchy")
    require(composer_api, ["$action==='create'", "compositionCreatePage", "includeInNavigation", "navigationAddPage"], "new-page composer API")
    ...
    for path in ["cms/composer.js", "cms/navigation.js", "cms/branding.js"]:
        ...
        if "http://" in source or "https://" in source:
            fail(f"{path} contains a third-party active runtime dependency")

    print("PASS: hierarchy, navigation, and branding contract")
